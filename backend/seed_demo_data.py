...
def _normalize_demo_project_examples(connection: Any, demo_storage: dict[str, Any]) -> None:
    ...
    def _is_food_sponsorship_item(item: dict[str, Any]) -> bool:
        text = " ".join(
            str(item.get(field) or "").strip().lower()
            for field in ("title", "name", "description", "category", "programModule", "program_id")
        )
        return "food sponsorship" in text or "sponsorship" in text
    ...
    if _table_exists(connection, "projects"):
        existing_projects = get_postgres_hot_storage_collection(connection, "projects")
        filtered_projects = [
            item for item in existing_projects
            if not (isinstance(item, dict) and _is_food_sponsorship_item(item))
        ]
        if len(filtered_projects) != len(existing_projects):
