"""
Add field officer task to events in the events table.
"""
import os
import json
import psycopg2
...
print("=== Adding Field Officer Task to Events ===\n")

# Get all events from events table
cursor.execute("""
    SELECT events_id, title, internal_tasks
    FROM events 
...
print(f"Found {len(events)} events in events table\n")
...
for event in events:
    ...
    updated_count += 1
    print(f"  ✓ Field officer task added ({len(updated_tasks)} total tasks)\n")
...
for event in events:
    ...
    print(f"  Field officer tasks: {field_officer_count}")
...
print("\n✅ All events now have field officer tasks!")
