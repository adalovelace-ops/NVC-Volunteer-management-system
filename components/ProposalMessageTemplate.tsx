...
export default function ProposalMessageTemplate({ application, onEdit, onSubmit, onOpenAttachment, onViewProjects, isAdmin, isOwner, isSubmitting }: Props) {
  ...
  if (d.proposedLocation && String(d.proposedLocation).includes(',')) {
    ...
    if (cityValue.length < 3) cityValue = 'Bacolod City';
  ...
  const startDate = formatDate(d.proposedStartDate || '2024-01-15');
  const endDate = formatDate(d.proposedEndDate || '2026-12-15');
  ...
  const photoName = photoUrl ? getFileName(photoUrl, 'mingo-_products.jpg') : 'mingo-_products.jpg';
