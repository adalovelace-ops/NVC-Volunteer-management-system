...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, ScrollView, Image, Platform, Linking, Alert } from 'react-native';
...
export default function AllReportsView({ reports, projects, volunteerTimeLogs = [], volunteers = [], onViewReport, reportType = 'all' }: Props) {
  ...
  const filteredReports = useMemo(() => {
    if (activeFilter === 'Events') return eventReports;
    if (activeFilter === 'Photos') return photoReports;
    return searchFiltered;
  }, [activeFilter, searchFiltered, eventReports, photoReports]);
  ...
  const folders = useMemo(() => {
    ...
    targetReports.forEach(rep => {
      ...
      const key = rep.projectId || rep.projectTitle || rep.category || 'uncategorized';
  ...
  const photoFolders = useMemo(() => {
    ...
    target.forEach(rep => {
      const key = rep.projectId || 'photos';
      const proj = rep.projectId ? projectById.get(rep.projectId) : undefined;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F8FAF6' }}>
      ...
      <View style={styles.showByRow}>
        <View style={styles.showByLeft}>
          <Text style={styles.showByLabel}>Show reports by</Text>
          <View style={styles.showByPills}>
      ...
      {(activeFilter === 'All' || activeFilter === 'Events') && (
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <View style={styles.sectionHeaderRight}>
              <Text style={styles.sectionMeta}>{folders.length} folders • {eventReports.length} reports</Text>
        ...
        {folders.length === 0 ? (
          ...
          <View style={styles.folderGrid}>
            {folders.map(folder => {
              ...
              return (
                <View key={folder.key} style={styles.folderCard}>
                  <View style={styles.folderTopRow}>
                    <View style={styles.folderIcon}>
                      <MaterialIcons name="folder" size={28} color="#EAB308" />
                    </View>
                    <TouchableOpacity style={styles.folderMenu} activeOpacity={0.7}>
        ...
        {(activeFilter === 'All' ? searchFiltered : eventReports).length === 0 ? (
          <View style={styles.emptyTable}>
            <Text style={styles.emptyTableText}>No reports found</Text>
          </View>
        ) : (
          (activeFilter === 'All' ? searchFiltered : eventReports).map(rep => {
            const proj = rep.projectId ? projectById.get(rep.projectId) : undefined;
      ...
      <View style={[styles.sectionCard, { marginTop: 16 }]}>
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.sectionHeaderRight}>
            <Text style={styles.sectionMeta}>{photoFolders.length} folder{photoFolders.length===1?'':'s'} • {photoReports.length} reports</Text>
        ...
        {photoFolders.length > 0 ? (
          <View style={styles.folderGrid}>
            {photoFolders.map(f => {
              ...
              return (
                <View key={f.key} style={styles.folderCard}>
                  <View style={styles.folderTopRow}>
                    <View style={styles.folderIcon}>
                      <MaterialIcons name="folder" size={28} color="#EAB308" />
                    </View>
