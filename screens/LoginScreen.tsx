...
export default function LoginScreen() {
  ...
  const handleSignup = async () => {
    ...
    if (signupEmail.trim() && !signupEmailVerified) {
      const errorMsg = "Please verify your email address before signing up.";
      setSignupValidationError(errorMsg);
      Alert.alert("Verification Required", errorMsg);
      return;
    }
                      ...
                      {signupRole === "admin" ? (
                        <>
                          ...
                          <View style={{ marginTop: 10, marginBottom: 15 }}>
                            <Text style={{ fontSize: 14, fontWeight: "bold", color: "#334155", marginBottom: 5 }}>Email verification</Text>
                            <View style={{ flexDirection: "row", alignItems: "center" }}>
                              ...
                              />
                              <TouchableOpacity
                                style={{ backgroundColor: signupEmailVerified ? "#10b981" : "#2563eb", paddingHorizontal: 15, paddingVertical: 12, borderRadius: 8, opacity: (!signupEmail.trim() || signupOtpLoading || signupOtpVerifying) ? 0.5 : 1 }}
                                onPress={signupEmailVerified ? undefined : (signupOtpSentEmail === signupEmail.trim().toLowerCase() ? handleVerifySignupOtp : handleSendSignupOtp)}
                                disabled={signupOtpLoading || signupOtpVerifying || !signupEmail.trim()}
                              >
                                {signupOtpLoading || signupOtpVerifying ? (
                                  <ActivityIndicator color="#fff" size="small" />
                                ) : (
                                  <Text style={{ color: "#fff", fontWeight: "bold", fontSize: 14 }}>
                                    {signupEmailVerified ? "Verified" : (signupOtpSentEmail === signupEmail.trim().toLowerCase() ? "Verify" : "Send Code")}
                                  </Text>
                                )}
                              </TouchableOpacity>
                            </View>
